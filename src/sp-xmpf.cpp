...
//			(dimensions X, Y, Z and with N grains eg filex100x100x15.000.dat for a grid of 100x100 with 15 grains)
	...
	nx = 100 * (planar or circle)
				+ dimensions[0]*(generated);
	ny = 100 * (planar or circle)
				+ dimensions[1]*(generated);
	
	num_grains += 2 * (planar or circle);
	
	if(planar){
		MMSP::grid<2,store_type > grid (2,0,nx,0,ny);
	
		MMSP::dx(grid, 0) = 0.01;//Lx/nx;
		MMSP::dx(grid, 1) = 0.01;//Ly/ny;
